...
public class XrayMain {
  ...
  public static void main(String[] args) throws IOException {
    ...
    switch (action) {
      ...
      case "upload" -> {
        if (scenario.contains(".feature")) {
          uploadXrayScenario(scenario);
        } else {
          uploadResult();
        }
      }
  ...
  private static String getXrayToken() throws IOException {
    ...
    RequestBody body =
        RequestBody.create(credentials.toString(), MediaType.get("application/json"));
  ...
  /**
   * Uploads scenario to XRAY.
   *
   * @param filename The name of the file containing scenario.
   * @throws IOException If an I/O error occurs.
   */
    ...
    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) {
        ...
        List<Map<String, String>> tests =
            new ArrayList<>(jsonPath.getList("updatedOrCreatedTests"));
   ...
   * @throws IOException If an I/O error occurs.
   */
  private static void uploadResult() throws IOException {
    System.out.println("Uploading test result from cucumber.json file...");
    File file = new File("build/cucumber.json");
    String token = getXrayToken();
    Request request = new Request.Builder()
        .url(XRAY_URL + "/import/execution/cucumber")
        .header("Content-Type", "application/json")
        .header("Authorization", "Bearer " + token)
        .post(RequestBody.create(file, MediaType.parse("application/json")))
        .build();
    try (Response response = client.newCall(request).execute()) {
      if (!response.isSuccessful()) {
        throw new IOException("Unexpected code " + response);
        ...
        if (response.code() != 200) {
          ...
          System.out.println("Successfully upload cucumber.json test result");
        }
      }
    }
  }
   ...
   * @throws IOException If an I/O error occurs.
   */
  private static void writeBuffertoFile(InputStream inputStream, File destinationFile)
      throws IOException {
